...
在原8点之外，新增了7个连续 NLO 边界点以及两个独立势参数族的 Yukawa
type 2/3/4（共6点）。所有点均由未修改的当前 `install/bin/CalcGW` 与关闭向量化的
v3.1.4 二进制直接重算；运行时最多并行两个 CalcGW。

- 新增13点的全部主状态、逐阶段状态和 transition history 翻转均为 `0/13`；合并
  原结果后为 `0/21`。
- 7个 NLO 连续点中有5个 GW success、1个 `not_set`、1个 NLO 拒绝；两个版本的
  退出阶段完全一致。5个成功点的 alpha 偏差为 `+10.44%` 至 `+21.26%`，beta/H
  偏差为 `-2.42%` 至 `+0.53%`，总 SNR 偏差为 `+58.98%` 至 `+129.47%`。这些
  SNR 的绝对值仍只有 `10^-26` 至 `10^-25`。
- Yukawa 新族2（L1=4.1292）的 type 2/3/4 均保持 success，alpha 约低
  `5.33%–5.44%`，总 SNR 约低 `7.99%–8.34%`，但绝对 SNR 约 `2e-26`。
- Yukawa 新族3（L1=3.2704）的 type 2/3/4 均保持 success，alpha 约低
  `28.40%–28.57%`，beta/H 相差不超过 `0.0063%`，总 SNR 约低
  `65.85%–66.12%`；绝对 SNR 从约 `8.1e-7` 降到 `2.8e-7`。
- 新增 NLO 批次累计 runtime 为 `771.476→1139.964 s`，当前版慢47.76%；新增
  Yukawa 批次为 `736.098→901.328 s`，当前版慢22.45%。这是路径相关结果，不能
  外推为统一速度比例。

扩展数据由 `summarize_expanded.py` 按列名生成 `expanded_result_summary.tsv`，避免宽表
使用固定列号时发生误读。新增结果强化了原结论：状态判断在已测范围内稳定，但
alpha 与 SNR 不存在可跨参数区使用的统一修正系数，且偏差方向和幅度均依赖参数族。
