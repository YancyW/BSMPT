...
本节仅在严格和v5全部完成后追加，未修改上面的规则或样本：

| 组 | 混淆 | 路由 | 含NLO前缀降时 |
|---|---:|---|---:|
| C1 type-2 | 4 TP，FP=FN=0 | 4/4近似接受 | 57.87% |
| C2 type-4 | 4 TP，FP=FN=0 | 4/4近似接受 | 58.73% |
| C3 coex B1 | 4 TN，FP=FN=0 | 4/4近似接受 | 21.49% |
| C4 coex C1 | 4 TN，FP=FN=0 | 4/4近似接受 | 24.77% |
| C5 bounce控制 | 2 TN，FP=FN=0 | 2/2严格回退 | 0.48% |
| C6 late控制 | 2 TN，FP=FN=0 | 2/2严格回退 | -1.65% |

全20点严格runtime合计`984.492 s`；v5输出`653.797 s`，严格NLO-only前缀
`6.397 s`，修正后`660.194 s`，总降时`32.94%`。正点最大SNR分量相对误差为
`1.0497%`。预注册验收全部通过；这只是预定义的20点分层确认面板，不是全空间证明。
