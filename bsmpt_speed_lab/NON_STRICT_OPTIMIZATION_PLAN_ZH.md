...
- N1a已完成：`compare_qualitative_outcomes.py`和`aggregate_qualitative_details.py`
  分别生成逐组与合并混淆矩阵、最早失败阶段和次级幅值统计。
- 旧42点当前裸近似：TP=20、TN=21、FP=1、FN=0，定性一致41/42；唯一错误为
  broad B第8行严格bounce failure而近似positive。
- 该结果不能外推，但证明旧guard 6/42接受主要是过度回退，而非36点都算错。
- 当前下一步为N1b：增加默认关闭的只读内部诊断，验证能否识别该假阳性和四点翻转
  邻域，并在不漏错的情况下认证部分稳定TN/TP。
- N1b首轮已完成：实验副本加入默认关闭的结构化bounce证书。broad B第8行FP在多个
  关键温度连续达到路径形变停滞门槛，broad A第3行TP对照能在关键温度进入原生
  `<0.05`收敛门槛。该观察仍只是候选特征，不能据两个样本上线。
- 当前下一步升级为N1c：四点翻转邻域与更多TP/TN结构化复核；同时开发仅在关键温度
  运行的legacy-gradient shadow。完整第二遍过慢，禁止把完整双跑伪装成优化。
- N1c四点邻域已完成：当前thermal-fast为2 FP + 1 TN + 1 FN，四点均被路径停滞
  证书标为unknown。该证书可防止危险点直接接受，但不能区分其中TN/FP/FN。
- 当前唯一下一步：实现legacy-gradient单/双关键温度bounce shadow；在完成前不得宣称
  非严格版已满足零定性翻转。
- C2单温度原型已实现：已捕获B8及邻域两个FP，单次成本0.89--1.54 s；但未捕获
  邻域FN。因此仅允许用来审查L1 positive，不允许据此接受bounce后的L1 fail。
- 下一步改为42点分层路由验证：早期明确失败、positive+shadow、bounce后失败三路分别
  核算。约3/42回退只是估算，必须经严格标签逐点验证后才可报告为结果。
- region-v1阶段验收已通过：42点完整wrapper为FP=FN=0、接受85.7%、exact 14.3%、
  端到端降低29.70%；独立高SNR局部E2的22点为FP=FN=0且降低59.24%。该结论只对
  已冻结压力集及该局部区域成立，不代表全参数空间。
- 后续目标从“证明存在可用安全域”升级为扩展多个相互独立的局部域和边界样本；任何
  新区域仍需先冻结再严格配对，不修改已经通过的region-v1规则。
