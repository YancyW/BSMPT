...
public:
  ...
  /**
   * Build immutable per-instance index lists for nonzero Higgs tensors.  The
   * lists contain indices only (never parameter- or field-dependent values),
   * and are built after SetCurvatureArrays has populated the tensors.
   */
  void BuildHiggsTensorIndexCache();

  /**
   * Build immutable per-instance index lists for the quark Yukawa tensors.
   * Only indices are cached; all parameter-, field-, and temperature-dependent
   * values continue to be read from the current tensors at evaluation time.
   */
  void BuildQuarkTensorIndexCache();

  /**
   * Build immutable per-instance index lists for the lepton Yukawa tensors.
   * Only indices are cached; all parameter-, field-, and temperature-dependent
   * values continue to be read from the current tensors at evaluation time.
   */
  void BuildLeptonTensorIndexCache();

  /**
   * Build an immutable per-instance hierarchy of nonzero tree-potential
   * tensor indices.  The hierarchy follows the original i,j,k,l nesting so
   * VTree can skip zero terms without changing the order of nonzero adds.
   */
  void BuildVTreeTensorIndexCache();

  /**
   * Build immutable per-instance index lists for nonzero gauge-Higgs
   * curvature tensor entries.  The mass list is ordered as the original
   * (i,j) contraction, while derivativeByField[i] is ordered by j for a
   * fixed field derivative i.
   */
  void BuildGaugeTensorIndexCache();

  /**
   * Build an immutable per-instance hierarchy of nonzero counterterm tensor
   * indices.  The hierarchy mirrors CounterTerm's i -> j -> k -> l order.
   */
  void BuildCounterTermIndexCache();

  // These caches are read-only after BuildHiggsTensorIndexCache() completes.
  // They are deliberately instance members: no tensor pattern is shared by
  // different model objects or parameter points.
  bool HiggsTensorIndexCacheReady = false;
  struct HiggsTensorIndexTerms
  {
    std::size_t k = 0;
    bool l3Nonzero = false;
    std::vector<std::size_t> l4Nonzero;
  };
  std::vector<std::vector<HiggsTensorIndexTerms>> HiggsTensorIndexCache;

  bool QuarkTensorIndexCacheReady = false;
  std::vector<std::vector<std::size_t>> QuarkMassFieldIndexCache;
  struct QuarkDerivativeIndexTerm
  {
    std::size_t i = 0;
    bool firstNonzero = false;
    bool secondNonzero = false;
  };
  std::vector<std::vector<QuarkDerivativeIndexTerm>>
      QuarkDerivativeIndexCache;

  bool LeptonTensorIndexCacheReady = false;
  std::vector<std::vector<std::size_t>> LeptonMassFieldIndexCache;
  struct LeptonDerivativeIndexTerm
  {
    std::size_t i = 0;
    bool firstNonzero = false;
    bool secondNonzero = false;
  };
  std::vector<std::vector<LeptonDerivativeIndexTerm>>
      LeptonDerivativeIndexCache;

  struct VTreeKIndexTerms
  {
    std::size_t k = 0;
    bool l3Nonzero = false;
    std::vector<std::size_t> l4Nonzero;
  };
  struct VTreeJIndexTerms
  {
    std::size_t j = 0;
    bool l2Nonzero = false;
    std::vector<VTreeKIndexTerms> kTerms;
  };
  struct VTreeIIndexTerms
  {
    bool l1Nonzero = false;
    std::vector<VTreeJIndexTerms> jTerms;
  };
  bool VTreeTensorIndexCacheReady = false;
  std::vector<VTreeIIndexTerms> VTreeTensorIndexCache;

  struct GaugeTensorIndexTerms
  {
    std::vector<std::pair<std::size_t, std::size_t>> mass;
    std::vector<std::vector<std::size_t>> derivativeByField;
  };
  bool GaugeTensorIndexCacheReady = false;
  std::vector<GaugeTensorIndexTerms> GaugeTensorIndexCache;

  struct CounterTermKIndexTerms
  {
    std::size_t k = 0;
    bool l3Nonzero = false;
    std::vector<std::size_t> l4Nonzero;
  };
  struct CounterTermJIndexTerms
  {
    std::size_t j = 0;
    bool l2Nonzero = false;
    std::vector<CounterTermKIndexTerms> kTerms;
  };
  struct CounterTermIndexTerms
  {
    bool l1Nonzero = false;
    std::vector<CounterTermJIndexTerms> jTerms;
  };
  bool CounterTermIndexCacheReady = false;
  std::vector<CounterTermIndexTerms> CounterTermIndexCache;

  // Flat counterpart of CounterTermIndexCache.  Entries are emitted in the
  // same i -> j -> k -> l order, but avoid traversing the hierarchy of small
  // vectors in the hot CounterTerm call.  `rank` is 2, 3, or 4 for L2, L3,
  // or L4 respectively; indices not used by a rank are left at zero.
  struct CounterTermFlatIndexTerm
  {
    std::size_t j = 0;
    std::size_t k = 0;
    std::size_t l = 0;
  ...
  bool CounterTermFlatIndexCacheReady = false;
  std::vector<std::vector<CounterTermFlatIndexTerm>>
      CounterTermFlatIndexCache;
 ...
 */
  ...
  std::vector<double> QuarkMassesSquared(const std::vector<double> &v,
                                         const int &diff = 0) const;
  void QuarkMassesSquared(const std::vector<double> &v,
                          const int &diff,
  ...
  std::vector<double> LeptonMassesSquared(const std::vector<double> &v,
                                          const int &diff = 0) const;
  void LeptonMassesSquared(const std::vector<double> &v,
                           const int &diff,
                           std::vector<double> &result) const;
