...
private:
  ...
  // Set only by the constructor without an analytic derivative.  The
  // derivative-provided constructor deliberately keeps the original path.
  bool UseCombinedNumericalDerivatives = false;
  bool UseActiveCombinedHessian = false;
  std::function<double(const std::vector<double> &)> NumericalPotential;
  double NumericalGradientOffset = 0;
...
public:
  ...
  // Interval hints are used only by the opt-in spline fast path.  They are
  // reset whenever either spline is rebuilt and never affect default runs.
  std::size_t RasterizeddVdlHint = 0;
  std::size_t PathLToXHint       = 0;
  std::size_t PathFieldHint      = 0;
  ...
  /**
   * @brief Numerical-derivative constructor with a const-reference potential
   * callback.  This opt-in form avoids copying the sampled VEV vector at the
   * BounceActionInt callback boundary.
   */
  BounceActionInt(
      std::vector<std::vector<double>> InitPath_In,
      std::vector<double> TrueVacuum_In,
  ...
  /** Calculate both path derivatives while reusing the common gradient. */
  std::pair<double, double> Calc_dVdlAndd2Vdl2(double l);
