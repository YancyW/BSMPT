...
TransitionTracer::TransitionTracer(user_input &input)
{
  ...
  const bool profile_enabled = std::getenv("BSMPT_PROFILE_PHASES") != nullptr;
  auto profile_last          = profile_clock::now();
  auto profile_stamp = [&](const char *label)
  {
    if (profile_enabled)
    {
      ...
      std::cerr << "BSMPT_PROFILE\t" << label << "\t" << seconds << '\n';
      profile_last = now;
    }
  };
  ...
  profile_stamp("minimum_tracer_construct");
          ...
          BSMPT::StatusNLOStability::Off)
  {
    ...
    if (not point_filtered_out_due_to_snr)
    {
      ...
      Vacuum vac(input.T_low,
                 ...
                 input.multistepmode,
                 input.num_points);
      profile_stamp("vacuum_phase_tracing");
        ...
        for (auto pair : vec_coex)
        {
              ...
              (pair.crit_status == BSMPT::StatusCrit::TrueLower))
          {
            ...
            BounceSolution bounce(input.modelPointer,
                                  ...
                                  input.PNLO_scaling);
            profile_stamp("bounce_construct");
            ...
            if (bounce.status_bounce_sol == StatusGW::Success)
            {
              ...
              profile_stamp("nucleation_approx");
