...
namespace
{
bool UseConstRefNumericalCallbackEnv() noexcept
{
  static const bool enabled = []
  {
    const char *value =
        std::getenv("BSMPT_USE_CONSTREF_NUMERICAL_CALLBACK");
    const char *combined =
        std::getenv("BSMPT_USE_COMBINED_NUMERICAL_DERIVATIVES");
    return value != nullptr && value[0] == '1' && combined != nullptr &&
           combined[0] == '1';
  }();
  return enabled;
}
...
void BounceSolution::GWInitialScan()
{
  ...
  for (double T = Tc - dT; T >= phase_pair.T_low + dT; T -= dT)
  {
    ...
    std::function<double(const std::vector<double> &)> V_ref =
        [&](const std::vector<double> &vev)
    {
      // Const-reference wrapper used only by the explicit fast-callback opt-in.
      return modelPointer->VEff(modelPointer->MinimizeOrderVEV(vev), T);
    };
    if (last_action < 0)
    {
    ...
    const bool use_analytic_gradient =
        std::getenv("BSMPT_USE_ANALYTIC_GRADIENT") != nullptr;
    const bool use_constref_numerical_callback =
        !use_analytic_gradient && UseConstRefNumericalCallbackEnv();
    std::function<std::vector<double>(std::vector<double>)> dV =
        [&](std::vector<double> vev)
    {
      const auto full_vev = modelPointer->MinimizeOrderVEV(vev);
      const auto &order   = modelPointer->Get_VevOrder();
      std::vector<double> result(order.size());
      for (std::size_t i = 0; i < order.size(); ++i)
        result[i] = modelPointer->VEff(full_vev, T, order[i] + 1);
      return result;
    };
    BounceActionInt bc = use_analytic_gradient
                             ? BounceActionInt(path,
                                               TrueVacuum,
                                               FalseVacuum,
                                               V,
                                               dV,
                                               T,
                                               MaxPathIntegrations)
                         : use_constref_numerical_callback
                             ? BounceActionInt(path,
                                               TrueVacuum,
                                               FalseVacuum,
                                               V_ref,
