...
size_t spline::find_closest_hint(double x, size_t hint) const
{
  const size_t n = m_x.size();
  if (n == 0 || x < m_x.front() || x > m_x.back()) return find_closest(x);
  if (hint >= n) hint = n - 1;
  while (hint + 1 < n && m_x[hint + 1] <= x) ++hint;
  while (hint > 0 && m_x[hint] > x) --hint;
  return hint;
}
...
double spline::operator()(double x, size_t &hint) const
{
  // Keep this arithmetic identical to operator()(double); only interval
  // lookup is replaced by the reusable hint.
  size_t n   = m_x.size();
  size_t idx = find_closest_hint(x, hint);
  hint       = idx;
...
double spline::deriv(int order, double x, size_t &hint) const
{
  assert(order > 0);
  size_t n   = m_x.size();
  size_t idx = find_closest_hint(x, hint);
