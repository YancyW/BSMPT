...
### A49. type-I晚期失败独立边界结果（2026-09-08）

- B10/A9新seed各6点均为TN、FP=FN=0，v7全部实际接受；修正降时分别60.94%和
  59.26%。
- A9严格为4 bounce + 2 nucleation，近似均为bounce，失败阶段漂移2/6，但正SNR/失败
  主结论一致。该事实不隐藏，也不作为放宽nucleation阈值的依据。
- 未出现10--11温度的未接受TN，因此预注册的v8触发条件不成立；v7保持≤6不变。
  新增12个严格独立点后累计270点：TP=96、TN=174、FP=FN=0。

### A50. type-I v7认证盒外沿对抗（2026-09-08）

- 在结果产生前以提交`9084eb19`冻结12点：A9的4点L4负向温度括号、4点联合外壳，
  以及B10的4点联合外壳；12/12命中late anchor且0/12命中direct-exact。
- strict为12 TN（10 bounce、2 nucleation），裸近似亦为12 TN，FP=FN=0。独立保存
  strict、raw approximate、route diagnostics和最终v7输出，避免fallback覆盖首遍证据。
- v7接受10点；A9的两个点因温度数10/11回退。Q1温度序列为6、6、6、10，未形成
  6/7相邻括号，不触发任何阈值或半径修改。
- strict runtime `1187.291 s`；v7输出`591.862 s`，加12个NLO前缀`4.723 s`后为
  `596.585 s`，降低49.75%。
- 纠正统计口径：此前270点是历史候选链累计，不是v7独立账本。加入本批后历史累计
  282点为TP=96、TN=186、FP=FN=0；直接当前v7晚期证书证据为44点，接受39、回退5，
  接受点均为TN。两者均不得外推为连续盒或完整type-I保证。
