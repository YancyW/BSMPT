...
状态：A15 已完成并淘汰，P4 耗尽。唯一未重复候选 quark 动态乘积 `noalias`
逐字段一致，但高 SNR type-3 慢 2.09%；实验开关随后移除。
...
状态：A16 已完成审计并停止。有限位置的 raster500/精确导数残差会漏掉窄峰、
零点移动和未经过区间；阈值取零才可保证零漏检，但等价于全部升级 raster1000。
...
A17 已证明全程序 PGO 会因未训练翻译单元缺少画像而失败。A18 仅允许把 PGO flags
施加到混合 R2HDM 训练确实产生 `.gcda` 的热点翻译单元；未选择对象正常编译，不把
缺失画像静默当作成功。选择集合必须在构建前固定，profile-use 全程零 missing、零
checksum/profile mismatch 警告，随后按固定验证阶梯验证。若无法建立对象到画像的
完整映射，或高 SNR 两次配对收益低于2%，立即停止。
