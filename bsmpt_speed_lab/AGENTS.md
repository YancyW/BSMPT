...
N1/N2和region-v1--v7阶段已经完成，累计历史候选链282个严格配对点为TP=96、
TN=186、FP=FN=0；该累计数不是全部由v7独立验证。当前直接v7晚期证书证据为44点，
接受39点。v7已在显式type-I锚点内用不同bounce温度数1--6接受稳定晚期失败；默认
下一步是独立扩展该证书边界及研究nucleation证书，不反向修改已冻结证据：

用户已指定后续只集中研究Yukawa type-I。既有type-II/III/IV结果只保留为历史记录，
不得继续为其扩样、调参或新增认证；新实验、算法优化和E3资源全部优先type-I。

1. 预先定义与E2锚点生成方式不同的独立方向、半径和样本数；
2. 在type-I内分层覆盖positive、NLO、coexistence、bounce和nucleation；
3. 分开报告裸近似、v7接受、exact回退、失败阶段漂移和端到端耗时；
4. 任一认证域出现FP/FN即撤销该域；域外回退不计为近似正确；
5. 低温度数失败证书不得脱离显式type-I锚点使用；nucleation温度数10--11仍回退，
   后续必须作为独立变量研究，不能直接放宽阈值。
