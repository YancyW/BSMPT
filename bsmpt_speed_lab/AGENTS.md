...
N1/N2和region-v1--v5阶段已经完成，累计240个严格配对点为TP=88、TN=152、
FP=FN=0；首个冻结E3面板20点已通过。默认下一步是扩大独立E3分层确认，不再用确认
结果反向修改v5：
...
2. 分层覆盖Yukawa 1--4、positive、NLO、coexistence、bounce和nucleation；
3. 分开报告裸近似、v5接受、exact回退、失败阶段漂移和端到端耗时；
4. 任一认证域出现FP/FN即撤销该域；域外回退不计为近似正确；
5. 晚期失败三温shadow在E22/E23慢约4%，不得加入v5；若优化该类，必须设计更便宜的
   单阶段证书并作为新变量重新验证。
