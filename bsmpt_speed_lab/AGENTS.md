...
N1/N2和region-v1--v6阶段已经完成，累计258个严格配对点为TP=96、TN=162、
FP=FN=0。默认下一步是扩大type-I独立E3分层确认，不再用确认结果反向修改v6：
...
3. 分开报告裸近似、v6接受、exact回退、失败阶段漂移和端到端耗时；
4. 任一认证域出现FP/FN即撤销该域；域外回退不计为近似正确；
5. 晚期失败三温shadow在E22/E23慢约4%，不得加入v6；若优化该类，必须设计更便宜的
   单阶段证书并作为新变量重新验证。
