...
N1a混淆矩阵已经完成。默认任务是非严格专项N1b，而不是继续降低固定raster或训练
参数空间分类器：

1. 在实验副本的`calcgw_profiler`、`RasterizedVdl`、`Solve1DBounce`、
   `CalculateActionAt`增加默认关闭、只记录已有量的诊断；
2. 用4个正常点证明诊断不改变任何非runtime输出且开销低于2%；
3. 用broad B第8行和四点邻域检查全部假阳性/假阴性；
4. 分析42点TP/TN与反例诊断分布，只有零漏错时才提出接受规则；
5. 完成N1b前，不实现N2额外求值、N3算法改变或N4上线router。
